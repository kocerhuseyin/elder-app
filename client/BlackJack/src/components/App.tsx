...
const App: React.FC = () => {
  ...
  return (
    <>
      <button
      type="button"
      style={{width: '150px', backgroundColor: 'transparent',border: 'none',
      height: '70px', fontSize: '20px', cursor: 'pointer'}}
      onClick={switchToApp}>
        <img src={home} alt="Home" style={{ width: '75px', height: '50px' }} />
      </button>
      ...
      />
      <Hand title={`Dağıtanın Eli (${dealerScore})`} cards={dealerCards} />
      <Hand title={`Elin (${userScore})`} cards={userCards} />
    </>
  );
}
